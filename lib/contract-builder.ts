...
export function buildReceiptPage(dev: ContractDevice, devIdx: number, isFirst: boolean, ctx: ContractCtx): string {
  ...
  if (isFirst) {
    ...
    if (idPhotoDataUrl) {
      p += `<div class="id-photo-wrap"><div class="id-photo-hd">📷 สำเนาบัตรประชาชน (มี Watermark)</div><img src="${idPhotoDataUrl}" alt="บัตรประชาชน"></div>`;
    }
    if (custProdPhotoDataUrl) {
      p += `<div class="id-photo-wrap"><div class="id-photo-hd">📸 รูปลูกค้าพร้อมสินค้า</div><img src="${custProdPhotoDataUrl}" alt="ลูกค้าพร้อมสินค้า" style="max-height:300px;object-fit:contain"></div>`;
    }
  }
