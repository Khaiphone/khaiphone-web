    ...
    <>
      ...
      <div style={{ padding: '32px 16px', display: 'flex', justifyContent: 'center' }}>
        <div className="page" style={{ background: '#fff', width: '100%', maxWidth: 720, padding: '48px 56px', boxShadow: '0 4px 24px rgba(0,0,0,0.12)', borderRadius: 4 }}>
          ...
          {/* Buyer */}
          <div style={{ marginBottom: 28, background: '#fffbeb', border: '1px solid #f59e0b', borderRadius: 8, padding: '14px 18px' }}>
            <p style={{ margin: '0 0 2px', fontSize: 11, color: '#92400e', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>ข้อมูลผู้ซื้อ / Buyer</p>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 24 }}>
            <thead>
              <tr style={{ background: '#B8860B', color: '#fff' }}>
                {['รายการ', 'รายละเอียด', 'ราคาก่อนภาษี', `VAT ${doc.vatRate}%`, 'ราคารวม'].map(h => (
                  <th key={h} style={{ padding: '10px 12px', textAlign: ['ราคาก่อนภาษี', `VAT ${doc.vatRate}%`, 'ราคารวม'].includes(h) ? 'right' : 'left', fontSize: 11, fontWeight: 600, letterSpacing: '0.04em' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              <tr style={{ borderBottom: '1px solid #e5e5e5' }}>
                <td style={{ padding: '14px 12px', fontSize: 14, fontWeight: 600, color: '#111', verticalAlign: 'top' }}>
                  {doc.model} {doc.storage}
                  {doc.color && <span style={{ color: '#666', fontWeight: 400 }}> · {doc.color}</span>}
                </td>
                <td style={{ padding: '14px 12px', fontSize: 12, color: '#555', verticalAlign: 'top' }}>
                  {doc.grade && <div>เกรด {doc.grade}</div>}
                  {doc.imei && <div>IMEI: {doc.imei}</div>}
                  {doc.serial && <div>S/N: {doc.serial}</div>}
                </td>
                <td style={{ padding: '14px 12px', textAlign: 'right', fontSize: 13, color: '#111', verticalAlign: 'top' }}>{fmt(baseAmount)}</td>
                <td style={{ padding: '14px 12px', textAlign: 'right', fontSize: 13, color: '#B8860B', verticalAlign: 'top' }}>{fmt(vatIncluded)}</td>
                <td style={{ padding: '14px 12px', textAlign: 'right', fontSize: 14, fontWeight: 700, color: '#111', verticalAlign: 'top' }}>{fmt(doc.amount)}</td>
              </tr>
            </tbody>
          </table>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 40 }}>
            <div style={{ width: 300 }}>
              {[
                { label: 'มูลค่าสินค้าก่อนภาษี', value: fmt(baseAmount), color: '#555' },
                { label: `ภาษีมูลค่าเพิ่ม ${doc.vatRate}%`, value: fmt(vatIncluded), color: '#B8860B' },
              ].map(row => (
                <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #e5e5e5' }}>
                  <span style={{ fontSize: 13, color: row.color }}>{row.label}</span>
                  <span style={{ fontSize: 13, color: row.color }}>{row.value}</span>
                </div>
              ))}
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderTop: '2px solid #B8860B', marginTop: 4 }}>
                <span style={{ fontSize: 15, fontWeight: 700 }}>ยอดรวมทั้งสิ้น</span>
                <span style={{ fontSize: 18, fontWeight: 800, color: '#B8860B' }}>{fmt(doc.amount)}</span>
              </div>
            </div>
          </div>
          ...
          {/* Note about VAT */}
          {doc.vatRate === 0 && (
            <p style={{ fontSize: 12, color: '#f59e0b', background: '#fffbeb', border: '1px solid #f59e0b', borderRadius: 6, padding: '8px 12px', marginBottom: 32 }}>
              ⚠ ยังไม่ได้ตั้งค่า VAT — ไปที่ตั้งค่าการเงิน → แท็บภาษี เพื่อเปิดใช้งาน VAT
            </p>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 40, gap: 24 }}>
            {[
              { label: 'ผู้ออกเอกสาร', name: doc.soldBy },
              { label: 'ผู้รับเอกสาร', name: doc.buyerName },
            ].map(({ label, name }) => (
