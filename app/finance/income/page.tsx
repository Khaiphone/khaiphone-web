...
import { incomeData } from '@/mock/finance/income'
...
const TABS = ['ทั้งหมด', 'ชำระแล้ว', 'รอยืนยัน']
...
export default function IncomePage() {
  ...
  const filtered = incomeData.filter((r: Transaction) => {
    const tabMatch =
      activeTab === 0 ||
      (activeTab === 1 && r.status === 'paid') ||
    ...
    <motion.div {...fadeUp} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      >
        ...
        >
          ...
          <input
            type="text"
            placeholder="ค้นหา เลขรายการ / ลูกค้า / สินค้า..."
            value={search}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${BORDER}` }}>
                {['วันที่', 'เลขรายการ', 'ลูกค้า', 'สินค้า', 'ช่องทาง', 'จำนวนเงิน', 'สถานะ', ''].map((h) => (
                  <th
                    key={h}
                    style={{
                      padding: '12px 16px',
              ...
              {paged.length === 0 ? (
                ...
                paged.map((r, idx) => (
                  ...
                  >
                    ...
                    <td style={{ padding: '12px 16px', color: '#FFFFFF', fontSize: 13 }}>{r.customer}</td>
                    <td style={{ padding: '12px 16px', color: TEXT2, fontSize: 13 }}>{r.product}</td>
                    <td style={{ padding: '12px 16px', color: TEXT2, fontSize: 13, whiteSpace: 'nowrap' }}>
                      {r.channel}
                    </td>
                    <td style={{ padding: '12px 16px', whiteSpace: 'nowrap' }}>
