...
export default function ProfitPage() {
  ...
  const { kpi, byModel } = data

  const profitKpi = [
    { label: 'รายได้รวม', value: kpi.totalRevenue, delta: 0, deltaType: 'positive' as const, format: 'currency' as const },
    { label: 'ต้นทุนรวม', value: kpi.totalCost, delta: 0, deltaType: 'negative' as const, format: 'currency' as const },
    { label: 'กำไรสุทธิ', value: kpi.netProfit, delta: 0, deltaType: kpi.netProfit >= 0 ? 'positive' as const : 'negative' as const, format: 'currency' as const },
    { label: 'Gross Margin', value: kpi.margin, delta: 0, deltaType: 'positive' as const, format: 'percent' as const },
  ]

  const profitDonut = byModel.slice(0, 7).map((m, i) => ({
    name: m.model,
    value: Math.max(0, m.totalProfit),
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ borderBottom: `1px solid ${BORDER}` }}>
                    {['รุ่น', 'เครื่อง', 'ต้นทุนเฉลี่ย', 'ราคาขายเฉลี่ย', 'กำไรเฉลี่ย', 'Margin %'].map((h) => (
                      <th key={h} style={{ padding: '10px 16px', textAlign: 'left', color: TEXT3, fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                  ...
                  {byModel.map((m, idx) => (
                    ...
                    >
                      ...
                      <td style={{ padding: '12px 16px', whiteSpace: 'nowrap' }}>
                        <span style={{ color: m.profitAvg >= 0 ? '#22c55e' : '#ef4444', fontWeight: 700, fontSize: 14 }}>
                          {m.profitAvg >= 0 ? '+' : ''}฿{m.profitAvg.toLocaleString('th-TH')}
                        </span>
                      </td>
          ...
          {profitDonut.length === 0 ? (
            ...
            <>
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  <Pie data={profitDonut} cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={3} dataKey="value">
