...
export async function fetchUnassignedJobs() {
  ...
  const { data } = await supabase
    .from("requests")
    .select("id, order_number, device_model, customer_name, customer_phone, appt_location, appt_date, appt_time, appt_lat, appt_lng, rider_id, rider_name, status, estimated_price, actual_price")
