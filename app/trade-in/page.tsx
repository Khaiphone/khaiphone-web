...
function getPrice(p: Product, cond: Condition) {
...
function RubSuePageContent() {
  const searchParams = useSearchParams();
  const activeTab = (searchParams.get("tab") as TabKey) || "iphone";
      ...
      <div className="max-w-6xl mx-auto px-4 py-4 flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            style={{ focusRingColor: "#B8860B" } as React.CSSProperties}
          />
          {search && (
      ...
      <div className="max-w-6xl mx-auto px-4 pb-8">
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {filtered.map(p => (
              <ProductCard key={p.id} p={p} condition={condition} />
            ))}
          </div>
        )}
      </div>
...
export default function RubSuePage() {
  return (
